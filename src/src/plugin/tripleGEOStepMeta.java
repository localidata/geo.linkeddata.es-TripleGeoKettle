...
public class tripleGEOStepMeta extends BaseStepMeta implements StepMetaInterface {
	...
	public void setDefault(){
		...
		try {
			String path = tripleGEOStepMeta.class.getProtectionDomain().getCodeSource().getLocation().getPath();
			String decodedPath = URLDecoder.decode(path, "UTF-8");			
			logMinimal("path "+decodedPath);
			String directory = decodedPath.substring(0, decodedPath.lastIndexOf("/"));
			logMinimal("directory "+directory);
			nameFile = directory + "/configuration.conf";
			logMinimal("nameFile "+nameFile);
